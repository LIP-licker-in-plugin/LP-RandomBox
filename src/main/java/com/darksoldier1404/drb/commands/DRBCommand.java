...
public class DRBCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        Player p;
        if (args.length == 0) {
            sender.sendMessage(plugin.prefix + "/drb 생성 <이름> - 랜덤 박스를 생성합니다.");
            sender.sendMessage(plugin.prefix + "/drb 보상 <이름> - 랜덤 박스 보상을 설정합니다.");
            sender.sendMessage(plugin.prefix + "/drb cmd <이름> <CustomModalData> - 랜덤 박스 쿠폰 아이템의 커스텀 모델 데이터를 설정합니다.");
            sender.sendMessage(plugin.prefix + "0으로 설정하면 삭제됩니다.");
            sender.sendMessage(plugin.prefix + "/drb 드랍 <이름> <수량> - 랜덤 박스 오픈시 지급할 보상 갯수를 설정합니다.");
            sender.sendMessage(plugin.prefix + "1 = 1슬롯, 2 = 2개의 슬롯의 아이템 지급");
            sender.sendMessage(plugin.prefix + "1개 2개의 아이템이 아닌 슬롯에 지정된 보상을 지급합니다.");
            sender.sendMessage(plugin.prefix + "/drb 쿠폰 <이름> - 랜덤 박스 쿠폰 아이템을 설정합니다.");
            sender.sendMessage(plugin.prefix + "/drb 쿠폰발급 <이름> (닉네임) - 자신 또는 대상에게 랜덤 박스 쿠폰 아이템을 발급합니다.");
            sender.sendMessage(plugin.prefix + "/drb 목록 - 랜덤 박스 목록을 표시합니다.");
            sender.sendMessage(plugin.prefix + "/drb 삭제 <이름> - 랜덤 박스를 삭제합니다.");
            sender.sendMessage(plugin.prefix + "/drb reload - config 설정 파일을 다시 불러옵니다.");
            return false;
        }
        if (args[0].equals("생성")) {
            if (args.length == 1) {
        ...
        if(args[0].equals("목록")) {
            DRBFunction.listRandomBoxs(sender);
            return false;
        }
        return false;
    }
